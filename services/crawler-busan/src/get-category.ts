...
const getCategoryList = async (): Promise<Category[]> => {
  const response = await fetch(
    "https://adventurebusan.lotteworld.com/kor/customer/lost-property/list.do"
  );
  ...
  const categoryOptionElements = document.querySelectorAll(
    "#acqrCategCd > option[value]"
  );

  const categoryOptions = categoryOptionElements
    .map(
      (element) =>
        ({
